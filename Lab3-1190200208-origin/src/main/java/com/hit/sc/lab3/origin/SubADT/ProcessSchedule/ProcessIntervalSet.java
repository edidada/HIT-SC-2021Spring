package com.hit.sc.lab3.origin.SubADT.ProcessSchedule;


import com.hit.sc.lab3.origin.ADT.MultiInterval.CommonMultiIntervalSet;
import com.hit.sc.lab3.origin.ADT.MultiInterval.MultiIntervalSet;
...
public class ProcessIntervalSet {
    ...
    private final List<java.lang.Process> processes = new ArrayList<>();  // 保存进程
    private final Map<java.lang.Process, Long> executedTime = new HashMap<>(); // 保存每个进程的已执行时间,键为Process，值为已执行时间
    ...
    public boolean addProcess(String info) {
        ...
        java.lang.Process p = new java.lang.Process(ID, name, minTime, maxTime);
    ...
    public boolean RAschedule() {
        ...
        List<java.lang.Process> temp_processes = new ArrayList<>(processes); // 保存尚未完全执行的进程
        while (!temp_processes.isEmpty()) {
            ...
            java.lang.Process p = temp_processes.get(random); // p为下一个执行的进程
    ...
    public boolean SPschedule() {
        ...
        while (!temp_processes.isEmpty()) {
            ...
            for (java.lang.Process temp : temp_processes) { // 找到最短的剩余时间
                long temp_remainTime = temp.getMaxTime() - executedTime.get(temp);
                remainTime = Math.min(temp_remainTime, remainTime);
            }

            java.lang.Process p = temp_processes.get(0); // p为下一个执行的进程

            for (java.lang.Process temp : temp_processes) {
                long temp_remainTime = temp.getMaxTime() - executedTime.get(temp);
    ...
    public void visualization() {
        ...
        if (processes.isEmpty())
            ...
            for (java.lang.Process p : processes) {
